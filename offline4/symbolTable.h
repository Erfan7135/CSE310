...
public:
 ...
 void print(){
    ...
    cout<<"Code"<<asmCode<<endl;
    cout<<"Var"<<asmVar<<endl<<endl<<endl;
 }
    ...
    void setAsFunction(vector<pair<string,string>> _paramList){ 
        paramList = _paramList;
        size = -1; 
    }

    void setAsArray(string type,string id, int size){
        Name = id;
        Type = type;
        this->size=size;
        next = NULL;
    ...
    void addParam(string type,string id){
        paramList.push_back(make_pair(type,id));
    }
...
void  print(std::ofstream& logout){
...
public:
    ...
    void printAll(std::ofstream& logout){
        scopeTable *temp = current;
        while(temp!=NULL){
            temp->print(logout);
            temp=temp->getParent();
